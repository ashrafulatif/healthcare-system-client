...
async function refreshTokenMiddleware(refreshToken: string) {
  // Call your refresh token API endpoint to get new tokens
  try {
...
export async function proxy(request: NextRequest) {
  try {
    ...
    //proactively refresh token if access token exprire or about to expire in next 5 minutes
    if (
      isValidAccessToken &&
      refreshToken &&
    ...
    if (pathname === "/reset-password") {
      ...
      //case 1 : if user needPasswordChange true
      ...
      //case 2 : user comming from forgot pass
      if (email) {
        return NextResponse.next();
      }
    ...
    //rule: user needPassword change and email not verfied.
    if (accessToken) {
      const userInfo = await getUserInfo();
      ...
      if (userInfo) {
        ...
        if (
          userInfo.needPasswordChange === false &&
